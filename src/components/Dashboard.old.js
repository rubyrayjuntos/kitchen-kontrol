        ...
        <div className="dashboard-wrapper p-6">
            ...
            <div className="card card-lg mb-6">
                <div className="card-header">
                    <h2 className="card-header-title text-2xl font-bold text-neumorphic-embossed flex items-center gap-2">
                        <BarChart3 size={24} />
                        Kitchen Status
                    </h2>
                </div>
                <div className="card-stats">
                    <div className="card-stat">
                        <span className="card-stat-value text-accent">{statusStats.activePhases}</span>
                        <span className="card-stat-label">Active Phases</span>
                    </div>
                    <div className="card-stat">
                        <span className="card-stat-value text-success">{statusStats.completedPhases}</span>
                        <span className="card-stat-label">Completed</span>
                    </div>
                    <div className="card-stat">
                        <span className="card-stat-value">{statusStats.pendingPhases}</span>
                        <span className="card-stat-label">Pending</span>
                    </div>
                    <div className="card-stat">
                        <span className="card-stat-value text-primary">
                            {Math.round((statusStats.completedTasks / statusStats.totalTasks) * 100)}%
                        </span>
                        <span className="card-stat-label">Tasks Complete</span>
                    </div>
                </div>
            </div>
            ...
            <div className="card card-lg">
                ...
                <div className="card-body">
                    <div className="phase-timeline neumorphic-raised p-4" role="list">
                        {phasesWithWidths.map((phase, index) => (
                            <div 
                                key={phase.id} 
...
export default Dashboard;
    ...
    // compute phase widths based on start times (minutes) so widths reflect durations
  const phasesWithWidths = React.useMemo(() => {
    try {
      const entries = Object.entries(scheduleData.phases).map(([id, p]) => ({ id, ...p }));
      const parseTime = (t) => {
        const m = (t || '').match(/(\d{1,2}):(\d{2})\s*(AM|PM)/i);
    ...
    <div className="dashboard-wrapper p-6">
      ...
      <div className="card card-lg mb-6">
        ...
        <div className="card-stats">
          ...
          <div className="card-stat">
            <span className="card-stat-value text-primary">{Math.round((statusStats.completedTasks / statusStats.totalTasks) * 100)}%</span>
            <span className="card-stat-label">Tasks Complete</span>
          </div>
        </div>
      </div>
